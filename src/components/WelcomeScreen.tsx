import { Monitor, AlertTriangle, Play, Loader } from 'lucide-react';
...
function WelcomeScreen({ onStart }: WelcomeScreenProps) {
    ...
    useEffect(() => {
        if (window.electronAPI) {
            const handleUpdateStatus = (_event: IpcRendererEvent, status: UpdateStatus) => {
                if (status.event === 'update-available' && status.data) {
                    window.electronAPI
                        .getAppVersion()
                        .then((currentVersion: string) => {
                            setUpdateInfo({
                                currentVersion,
                                newVersion: status.data?.version,
                                releaseName: status.data?.releaseName,
            ...
            window.electronAPI.checkForUpdates().catch(() => setIsCheckingUpdate(false));
    ...
    if (updateInfo && isUpdateModalOpen) {
        return <UpdateInfoModal updateInfo={updateInfo} />;
    }
        ...
        return (
            <div className="min-h-screen flex items-center justify-center p-6">
                <div className=" flex items-center gap-2 text-center">
                    ...
                    <p className="text-gray-400">Đang kiểm tra phiên bản mới...</p>
                </div>
            </div>
        );
    }
